...
import { ADD_POST_FAIL, ADD_POST_REQUEST, ADD_POST_RESET, ADD_POST_SUCCESS, EDIT_POST_FAIL, EDIT_POST_REQUEST, EDIT_POST_RESET, EDIT_POST_SUCCESS } from '../../../../redux/constants/postsConstants'
...
const PostFrom = () => {
  ...
  const { loading:updateLoading, success:updateSuccess, error:updateError } = updatePostST
  ...
  return (
    <div>
       { success && <Alert onClose={() => {dispatch({type:ADD_POST_RESET})}}>تم النشر بنجاح</Alert> }
      { error && <Alert variant='error' onClose={ () => { dispatch({ type: ADD_POST_RESET }) } }>{ error }</Alert> }
      { updateSuccess && <Alert onClose={() => {dispatch({type:EDIT_POST_RESET})}}>تم التعديل بنجاح</Alert> }
       { updateError && <Alert variant='error' onClose={ () => { dispatch({ type: EDIT_POST_RESET }) } }>{ updateError }</Alert> }
        <h1 className='my-5 text-center'>إضافة منشور</h1>
        <form className='w-50 mx-auto' dir='rtl' onSubmit={onSubmitHandler}>
            <div className='form-group'>
                <label htmlFor='post-content' className='form-label'>محتوى المنشور</label>
                <textarea className='form-control' required value={text} onChange={(e)=>setText(e.target.value)} name='post-content mt-2'></textarea>
            </div>
            <div className='form-group'>
              <label htmlFor='form-file mb-2' className='form-label'>صورة</label>
              <input type='file' className='form-control' id='img-post' name='form-file'  accept='.png,.jepg,.jpg'
                onChange={(e) => {
              ...
              <div className={classes.shareImgContainer} >
                <img
                  ...
                  alt='My post'
                />
              <Cancel className={ classes.shareCancelImg } fontSize='large'
                onClick={
                  () => { setFile(null);document.getElementById('img-post').value=''}
