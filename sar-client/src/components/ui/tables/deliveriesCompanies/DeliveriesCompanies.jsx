...
const DeliveriesCompanies = ({ data }) => {
  ...
  const dispatch = useDispatch()
    ...
    const { deliveryCompanies: deliveryCompaniesList, error, loading } = deliveryCompanies;

    const deleteDeliveryCompanySt = useSelector((state) => state.deleteDeliveryCompany);
    const { success, error: deleteError, loading: deleteLoading } = deleteDeliveryCompanySt;

    const addDeliveryCompanySt = useSelector((state) => state.addDeliveryCompany);
    const { success:addSuccess, error: addError, loading: addLoading } = addDeliveryCompanySt;
  ...
  return (
    <div className='text-center'>
      ...
      { success && <Alert onClose={() => {dispatch({type:DELETE_DELIVERY_COMPANY_RESET})}}>تم الحذف بنجاح</Alert> }
      { deleteError && <Alert variant='error' onClose={ () => { dispatch({ type: DELETE_DELIVERY_COMPANY_RESET }) } }>{ deleteError }</Alert> }
          
      <h3 className='my-5 fw-bold'>شركات التوصيل </h3>
      <div className='table-responsive'>
            <table className='table table-bordered table-striped w-75 mx-auto mt-2' dir='rtl'>
              <thead>
                  ...
                  <tr key={DC.deliveryID}>
                    ...
                    <td className='d-flex justify-content-center'>
                      ...
                      <button
                        ...
                        onClick={
                              () => {
                                  dispatch({ type: SET_CURRENT_DELIVERY_COMPANY, payload: DC }),
                      ...
                      <FormControlLabel
                                  control={
                                      <Checkbox 
                                            defaultChecked ={ DC.cstate != 'deleted' }
                                            name="Show"
                                            onClick={ DC.state != 'deleted' ?
