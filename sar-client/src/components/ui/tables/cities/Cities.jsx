...
import { deleteCity, getCities, hideCity, showCity } from '../../../../redux/actions/citiesActions.js'
import { Box } from '@mui/system'
import { ADD_CITY_RESET, DELETE_CITY_RESET, SET_CURRENT_CITY, SET_CURRENT_CITY_RESET, UPDATE_CITY_RESET } from '../../../../redux/constants/citiesConstants.js'
...
const Cities = ({ data }) => {
    ...
    const { success:addSuccess, error: addError, loading: addLoading } = addCitySt;
  ...
  return (
      <div>
          ...
          { success && <Alert onClose={() => {dispatch({type:DELETE_CITY_RESET})}}>تم الحذف بنجاح</Alert> }
          { deleteError && <Alert variant='error' onClose={ () => { dispatch({ type: DELETE_CITY_RESET }) } }>{ deleteError }</Alert> }
          
          <h3 className='text-center fw-bold mb-3 mt-5'>المدن</h3>
          <div className='table-responsive'>
                <table className='table table-bordered table-striped w-75 mx-auto' dir='rtl'>
                    <thead>
                        ...
                        { citiesList.map(city => (
                            <tr key={city.cityID}>
                                ...
                                <td className='d-flex justify-content-center'>
                                  ...
                                  control={
                                      <Checkbox 
                                            defaultChecked ={ city.state != 'deleted' }
                                            name="Show"
                                            onClick={ city.state != 'deleted' ?
