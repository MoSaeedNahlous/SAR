import { Alert, Box, Button, Checkbox, CircularProgress, FormControl, FormControlLabel, InputLabel, MenuItem, Select } from '@mui/material'
import React, { useEffect, useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { getCategories, hideCategory, showCategory } from '../../../../redux/actions/categoriesActions'
import {getSizes, hideSize, showSize,deleteSize } from '../../../../redux/actions/sizesActions'
import { getSubCategories } from '../../../../redux/actions/subCategoriesActions'
import { ADD_SIZE_RESET, DELETE_SIZE_RESET, SET_CURRENT_SIZE } from '../../../../redux/constants/sizesConstants'
...
const Sizes = ({ data }) => {
  ...
  const { success: deleteSuccess, error: deleteError, loading: deleteLoading } = deleteSizeSt;
  ...
  if (sizeLoading) {
    ...
    if (sizeError) {
        return <Alert variant='error'>{sizeError }</Alert>
  }
  
  return (
    <div className='text-center mt-3'>
          { sizeError && <Alert variant='error'>{ sizeError }</Alert> }
          { deleteSuccess && <Alert onClose={() => {dispatch({type:DELETE_SIZE_RESET})}}>تم الحذف بنجاح</Alert> }
          { deleteError && <Alert variant='error' onClose={ () => { dispatch({ type: DELETE_SIZE_RESET }) } }>{ deleteError }</Alert> }
          <h2>القياسات</h2>
          ...
          >
          {catLoading ? <CircularProgress size={ 100 } color='grey' /> : catError ?
            <Alert variant='error'>{sizeError }</Alert>:
            categories.map((cat) => (
              <MenuItem
                key={ cat.catID }
                value={ cat.catID }
        ...
        <Select
          ...
          { subCatLoading ?
            <CircularProgress size={ 100 } color='grey' /> :
            subCatError ?
              <Alert variant='error'>{ subCatError }</Alert> :
                    ...
                    <tr
                      key={ size.sizeID }>
                        ...
                        <FormControlLabel
                          control={
                                <Checkbox 
                                            defaultChecked ={ size.state != "deleted" }
                                            name="Show"
                                            onClick={ size.state != "deleted" ?
