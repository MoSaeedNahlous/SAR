...
export const getCustomers = (name, mobile) => async (dispatch) => {
  try {
    // loading
    dispatch({
    ...
    const { data } = await axios.get(
      name || mobile
        ? `/api/User?level=searchName&empid=0&username=${name}&mobile=${mobile}`
        : `/api/User?empid=0&username=H&level=select`
    );
...
export const getCustomersE = (empId, name, mobile) => async (dispatch) => {
  try {
    // loading
    dispatch({
      type: GET_CUSTOMERS_REQUEST,
    ...
    const { data } = await axios.get(
      name || mobile
        ? `/api/user?level=searchName&empid=${empId}&username=${name}&mobile=${mobile}`
        : `/api/user?level=selectByEmployee&empid=${empId}`
    );
...
export const addNewCustomer =
  (name, add1, add2, mob1, mob2, cityId, notes, url, empId) =>
  async (dispatch) => {
    try {
      dispatch({
...
export const updateCustomer =
  (id, name, add1, add2, mob1, mob2, cityId, notes, url) =>
  async (dispatch) => {
    try {
      dispatch({
