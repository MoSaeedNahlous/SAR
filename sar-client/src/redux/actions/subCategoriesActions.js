...
export const getSubCategories = (catId) => async (dispatch) => {
  try {
    ...
    const { data } = catId
      ? await axios.get(`/api/subCategories?level=SelectbyCatNM&CatID=${catId}`)
      : await axios.get(`/api/subCategories?level=selectM&CatID=${1}`);
...
export const getSubCategoriesDisplay = (catId) => async (dispatch) => {
  try {
    dispatch({
      type: GET_SUBCATEGORIES_REQUEST,
    });
    ...
    const { data } = await axios.get(
      `/api/subCategories?level=SelectbyCatN&CatID=${catId}`
    );
...
export const addNewSubCategory = (catId, name) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`/api/subCategories`, body);
...
export const deleteSubCategory = (subCatId) => async (dispatch) => {
  try {
    ...
    if (data.table[0].column1 === 'cant delete it') {
      throw new Error('لا يمكن حذف الصنف لوجود أصناف معلومات متعلقة فيه');
    }
...
export const updateSubCategory =
  (catId, name, subCatId) => async (dispatch) => {
    try {
      dispatch({
